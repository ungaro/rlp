    ...
    Ok(quote! {
        const _: () = {
            ...
            impl #impl_generics alloy_rlp::RlpEncodable for #name #ty_generics #where_clause {
                #[inline]
                fn length(&self) -> usize {
                    alloy_rlp::RlpEncodable::length(&self.#ident)
                }
...
fn encodable_length<'a>(
    ...
    if is_opt {
        ...
        quote! { self.#ident.as_ref().map(|val| alloy_rlp::RlpEncodable::length(val)).unwrap_or(#default) }
    } else {
        quote! { alloy_rlp::RlpEncodable::length(&self.#ident) }
    }
}
